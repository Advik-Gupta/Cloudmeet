<script lang="ts">
  ...
  let bookingStatus = $state<"idle" | "submitting" | "success" | "error">(
    "idle"
  );
  ...
  async function fetchMonthAvailability() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/availability/month?event=${data.slug}&month=${monthStr}`
      );
  ...
  async function handleDateSelect(dateStr: string) {
    ...
    try {
      const response = await fetch(
        `/api/availability?event=${data.slug}&date=${dateStr}`
      );
      if (!response.ok) throw new Error("Failed to fetch availability");
...
<div
  ...
  style="--brand-color: {brandColor}; --brand-light: {colors.light}; --brand-lighter: {colors.lighter}; --brand-dark: {colors.dark}; --brand-rgb: {colors
    .rgb.r}, {colors.rgb.g}, {colors.rgb.b};"
>
  ...
  {:else}
    ...
    <div class="md:hidden min-h-screen w-full bg-white">
      ...
      {#if data.eventType?.cover_image}
        <div class="px-6 pt-6 flex justify-center">
          <img
            src={data.eventType.cover_image}
            alt=""
      ...
      {#if mobileStep === "calendar"}
        <div class="flex flex-col items-center pt-8 pb-6 px-6">
          ...
          {:else}
            <div
              class="w-24 h-24 rounded-full flex items-center justify-center text-white font-semibold text-3xl border-4 border-white shadow-lg"
              style="background-color: var(--brand-color)"
            >
              {data.user?.name?.charAt(0) || "M"}
            </div>
          {/if}
          <p class="mt-4 text-base font-semibold text-gray-600">
            {data.user?.name || "Host"}
          </p>
        </div>
        ...
        <div class="px-6 pb-5">
          <ul class="space-y-3 text-sm text-gray-600">
            ...
            <li class="flex items-center gap-3">
              ...
              <span
                >{data.eventType?.invite_calendar === "outlook"
                  ? "Microsoft Teams"
                  : "Google Meet"}</span
              >
            </li>
        ...
        {#if data.eventType?.description}
          <div
            class="px-6 pb-5 text-sm text-gray-600 prose prose-sm max-w-none"
          >
        ...
        <div class="px-6 pb-8">
          <h2 class="text-lg font-semibold text-gray-900 mb-5 text-center">
            Select a Date & Time
          </h2>
          ...
          <div class="grid grid-cols-7 gap-1">
            {#each calendarDays() as day}
              ...
              <button
                ...
                style="{isClickable && !isSelected
                  ? `background-color: var(--brand-lighter); color: var(--brand-dark)`
                  : ''}{isSelected
          ...
          {#if loading}
            <div class="flex items-center justify-center py-8">
              <div
                ...
                style="border-color: var(--brand-color); border-top-color: transparent;"
              ></div>
            </div>
      ...
      {#if mobileStep === "form"}
        <div class="px-6 pb-8">
          ...
          <p class="text-sm text-gray-500 text-center mb-6">
            {selectedDate ? formatShortDate(selectedDate) : ""}{selectedSlot
    ...
    <div
      class="hidden md:flex bg-white rounded-2xl shadow-lg overflow-hidden transition-all duration-300 ease-in-out"
      style="width: {showForm ? '700px' : selectedDate ? '920px' : '650px'}"
    >
      <!-- Left Sidebar -->
      <EventSidebar
        user={data.user}
      ...
      <div class="flex-1 p-6">
        {#if bookingError}
          <div
            class="bg-red-50 border border-red-200 text-red-800 rounded-lg p-4 mb-6 max-w-2xl"
          >
            {bookingError}
          </div>
        {/if}
